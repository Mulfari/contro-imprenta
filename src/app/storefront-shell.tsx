                      ...
                      {group.items.map((item) => {
                        ...
                        return (
                          <button
                            ...
                            className="flex w-full cursor-pointer items-center justify-between rounded-xl bg-white px-3 py-2.5 text-left text-sm text-slate-700 transition hover:border-slate-200 hover:bg-slate-950 hover:text-white"
                          >
                            <span>{item}</span>
              ...
              {filteredProducts.length > 0 ? (
                <div className="mt-6 grid gap-4 md:grid-cols-2 xl:grid-cols-3">
                  {filteredProducts.map((item) => (
                    <article
                      key={item.title}
                      className="group overflow-hidden rounded-[1.7rem] border border-slate-200 bg-white shadow-[0_12px_30px_rgba(15,23,42,0.04)] transition hover:-translate-y-1 hover:border-slate-300 hover:shadow-[0_22px_40px_rgba(15,23,42,0.08)]"
                    >
                      <div
                        className={`relative bg-gradient-to-br ${item.tint} p-5`}
                      >
                        <div className="flex items-start justify-between gap-3">
                          <span className="rounded-full bg-white/80 px-3 py-1 text-xs font-semibold text-slate-700">
                            {item.category}
                          </span>
                          <span className="rounded-full border border-white/60 bg-white/50 px-3 py-1 text-xs font-semibold text-slate-600">
                            Disponible
                          </span>
                        </div>

                        <div className="mt-5 flex h-36 items-end justify-between rounded-[1.15rem] border border-white/55 bg-white/40 p-4">
