...
function ActiveProductPreparation({
  ...
  return (
    <section className="rounded-[2rem] border border-slate-200 bg-white p-5 shadow-[0_20px_48px_rgba(15,23,42,0.05)] sm:p-6">
      <div className="flex flex-col gap-4 lg:grid lg:grid-cols-[15rem_1fr]">
        <div className={`flex min-h-56 items-center justify-center overflow-hidden rounded-[1.6rem] bg-gradient-to-br ${item.product.tint} p-5`}>
          <Image
            ...
            height={320}
            sizes="18rem"
            className="h-auto max-h-52 w-auto max-w-full object-contain drop-shadow-[0_20px_34px_rgba(15,23,42,0.18)]"
          />
        </div>
      ...
      <div className="mt-6 rounded-[1.55rem] border border-slate-200 bg-slate-50 p-4 sm:p-5">
        <div className="flex flex-col gap-4 lg:grid lg:grid-cols-[1fr_17rem]">
          <label className={`block rounded-[1.35rem] border border-dashed px-4 py-5 text-center transition ${
            prep.sendLater
              ? "border-slate-200 bg-white text-slate-400"
              : "cursor-pointer border-slate-300 bg-white text-slate-700 hover:border-slate-400"
          }`}>
            <span className="block text-sm font-black text-slate-950">
              Subir arte de este producto
            </span>
            <span className="mt-1 block text-xs leading-5 text-slate-500">
              Puedes adjuntar uno o varios archivos. Cada producto se revisa por separado.
            </span>
            <input
              type="file"
              multiple
        ...
        <div className="mt-4 grid gap-3 lg:grid-cols-[1fr_17rem]">
          <div className="rounded-[1.25rem] border border-slate-200 bg-white p-4">
            <p className="text-sm font-black text-slate-950">Arte seleccionado</p>
            <div className="mt-3 space-y-2">
              {prep.files.length > 0 ? (
                ...
                <p className="rounded-xl border border-dashed border-slate-300 bg-slate-50 px-3 py-3 text-sm text-slate-500">
                  {prep.sendLater ? "Arte pendiente para despues." : "Aun no has seleccionado arte."}
                </p>
              )}
            </div>
          </div>
          ...
          <textarea
            ...
            className="w-full resize-none rounded-[1.25rem] border border-slate-200 bg-white px-4 py-3 text-sm text-slate-900 outline-none transition placeholder:text-slate-400 focus:border-slate-400"
          />
        </div>
      </div>
